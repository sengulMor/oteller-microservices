...
class GlobalExceptionHandlerTest {
    ...
    void handleRoomNotFoundException_shouldReturnNotFoundMessage() {
        Long id = 1L;
        ReservationNotFoundException ex = new ReservationNotFoundException(id);
        ResponseEntity<String> response = handler.handleReservationNotFoundException(ex);
        assertEquals(HttpStatus.NOT_FOUND, response.getStatusCode());
        assertNotNull(response.getBody());
        assertEquals("Reservation with ID " + id + " not found", response.getBody());
    }
